...
% "A new transformation technique for solving a class of highly linear systems of equations via evolutionary algorithm".
% The purpose is didactic and aims to exemplify mainly how to use the fitness function proposed in the article.
% To illustrate, we are applying the technique to the Combustion Application problem (one of the systems proposed in the article).
...
y(10)= 0.2089296*(10^(-14))*x(10)-x(1)*x(2);% f10
endfunction
...
x(p,6:10) = M\S(x(p,1:5)); % Given x1, x2, x3, x4 and x5, we are determining the variables x6 to x10 (coordinates to coordinates of W in the text of the aforementioned article).
